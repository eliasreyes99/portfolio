...
export default function Home() {
  ...
  const sendEmail = (e) =>{
    e.preventDefault();
    emailjs.sendForm('service_3gcha69','template_minf2rk', form.current, 'hRLAM9mKWOrcWw8nF')
    .then((result) => {
      console.log(result.text);
      setAlert({description:'Email sent', status:'alert-success'});
    },(error) => {
      console.log(error.text);
      setAlert({ description:'Error to send email', status:'alert-error' })
    });
  };
  return (
        ...
        <SimplePopover title="Send Mail">
          <div className="sendMail" >
            <form ref={form} onSubmit={sendEmail}>
              ...
              <button type="submit" value="Enviar">Send Mail</button>
            </form>
          </div>
        </SimplePopover>
      </div>
